...
export default function App() {
  ...
  function clear() { setDisplay('0'); setStored(null); setPending(null); setWaiting(false); setEvaluated(false); setLastOperation(null); setLastOperand(null); setMessage(null); }
  ...
  async function chooseOperation(operation: Operation) {
    clearMessage();
    if (operation === 'sqrt') { setStored(null); setPending('sqrt'); setWaiting(false); setEvaluated(false); return; }
    if (!BINARY_OPERATIONS.includes(operation) || busy) return;
    if (pending && stored !== null && !waiting) {
      const operands = [stored, display];
      const validation = localValidation(pending, operands);
      if (validation) { setMessage(validation); return; }
      setBusy(true);
      try {
        const value = formatDisplay(await calculate(pending, operands));
        setDisplay(value); setStored(value); setPending(operation); setWaiting(true); setEvaluated(false);
      } catch (error) { setMessage(error instanceof Error ? error.message : 'The calculation could not be completed. Try again.'); }
      finally { setBusy(false); }
      return;
    }
    setStored(display); setPending(operation); setWaiting(true); setEvaluated(false);
  }

  async function equals() {
    if (busy) return;
    const operation = pending ?? (evaluated ? lastOperation : null);
    if (!operation) return;
    const operands = pending === 'sqrt' ? [display] : pending ? [stored || '0', display] : [display, lastOperand || '0'];
    const validation = localValidation(operation, operands);
    if (validation) { setMessage(validation); return; }
    setBusy(true); setMessage(null);
    try {
      const value = await calculate(operation, operands);
      setDisplay(formatDisplay(value)); setStored(null); setPending(null); setWaiting(false); setEvaluated(true);
      if (operation !== 'sqrt') { setLastOperation(operation); setLastOperand(operands[1]); }
    }
    catch (error) { setMessage(error instanceof Error ? error.message : 'The calculation could not be completed. Try again.'); }
    finally { setBusy(false); }
  }
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (operation[event.key]) { event.preventDefault(); void chooseOperation(operation[event.key]!); }
    }
  ...
  return <main className="calculator-page" aria-label="Traditional calculator">
    <section className="calculator-wrap" aria-label="Calculator work surface">
      ...
      <div className="keypad" aria-label="Calculator keypad">
        {button('⌫', backspace, 'utility backspace', 'Backspace')}{button('C', clear, 'utility', 'Clear calculator')}{button('%', () => void applyPercentage(), 'utility operator', 'Percentage')}{button('÷', () => void chooseOperation('divide'), 'operator', 'Divide')}
        {button('7', () => inputDigit('7'))}{button('8', () => inputDigit('8'))}{button('9', () => inputDigit('9'))}{button('×', () => void chooseOperation('multiply'), 'operator', 'Multiply')}
        {button('4', () => inputDigit('4'))}{button('5', () => inputDigit('5'))}{button('6', () => inputDigit('6'))}{button('−', () => void chooseOperation('subtract'), 'operator', 'Subtract')}
        {button('1', () => inputDigit('1'))}{button('2', () => inputDigit('2'))}{button('3', () => inputDigit('3'))}{button('+', () => void chooseOperation('add'), 'operator', 'Add')}
      ...
      <div className="scientific" aria-label="Additional operations"><div className="scientific-keys">{(['power', 'sqrt'] as Operation[]).map((operation) => button(symbols[operation], () => void chooseOperation(operation), 'scientific-key', labels[operation]))}</div></div>
